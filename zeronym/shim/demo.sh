...
# and sends five calls through, so you see the classifier's verdicts and the
...
run_offline() {
    ...
    note "Five SendTransaction calls: a migration, a non-migration, garbage,"
    note "a compressed body, and one ordinary proxied method. Watch the"
    note "zis::classify verdicts. All five are forwarded: this PoC does not divert."
    echo
...
run_live() {
    ...
    rule "2/3  SendTransaction carrying a real V6 Orchard -> Ironwood migration"
