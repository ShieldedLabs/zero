...
| **current**, hub-hop ALPN fix | `3e9e1cec7a74f55d66f1bbe7eb4d29534302a38d59310579db5fa0ea711a360c` | the hub hop now negotiates `http/1.1` instead of `h2` (`BackendTls::new_http1`). Found in production: the hub's ALPN-honouring Caddy agreed to h2 and waited for a preface our HTTP/1.1 client never sends, so every diverted migration failed closed as "hub unreachable" over a valid TLS session. |
