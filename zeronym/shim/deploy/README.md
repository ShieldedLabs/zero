...
|---|---|---|
| **current**, review hardening merged onto the 0.4.0 stack | `7375176ddcf482ead8b726f2fff70a48fad0f61f8da22e81a3576539aae23591` | the zaino-proto 0.4.0 stack (row below) merged with main's shim hardening, landed while that row was in review: the Hornby-review mitigations (a `--require-diversion` mode, a 1 KiB `TxFilter` cap replacing the shared 4 MiB one, a 30 s body-read ceiling) and the bounded inbound listeners. Measured across FOUR cold builds on the x86_64 runner: two runs of two builds each, hours apart, all agreeing. |
