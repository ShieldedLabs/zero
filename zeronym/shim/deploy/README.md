...
|---|---|---|
| **current**, Caution control-plane paths | `8b5ec3fa2365153d78d8d16de911bc1283633bd7dcb057ac36d7b5870f782b6d` | `route_for` owns `/.well-known/caution/health` and `/attestation` instead of proxying them to the indexer, so compiled code changed. ONE build on one host (arm64 under Rosetta) so far: not yet cross-machine confirmed, unlike every row below it. Dispatch `zeronym-shim-reproduce` on the commit that carries this value before treating it as published. |
