//! The shim-to-hub wire frames, version 1: `SubmitV1` out, `AckV1` back.
//!
...
//!   (the txid is a control input an adversary could write; the shim computes it
//!   itself from the bytes). The nonce is echoed in the ack and matched there.
//!
//! ```text
...
//! [`WireError`] (the hub answers those as `bad_frame`), but the bytes past the
//! declared transaction are never read, so nothing downstream can smuggle meaning
//! into the padding.
