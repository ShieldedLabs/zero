...
- **~~Reproducible-attestation gap (PCR0/PCR1).~~ CLOSED 2026-08-17, and the earlier framing was backwards.** All three PCRs reproduce: hub-6 and shim-9 both verified `PASSED` from fresh public clones with the build cache cleared, including the TLS certificate binding. The old entry said the application's measurement was PCR2 and that PCR2 "carries the weight"; measured 2026-08-14 the opposite holds -- hub and shim, two entirely different binaries, produced byte-identical PCR2 (`21b9efbc...`) while PCR0/PCR1 tracked the application. PCR2 does not identify the code, so a PCR2-only acceptance would prove only that *some* Caution enclave is running. Require all three (OPERATORS.md).
- **Which PCR index measures which layer (for Caution).** The above is an empirical observation from two applications, not a confirmed mapping. Worth one question to Caution so the trust documentation can state it rather than infer it.
