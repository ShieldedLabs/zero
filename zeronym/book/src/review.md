...
- **The classifier boundary: net or gross?** The sharpest question the PoC surfaced. The predicate is strict: V6 **and** `orchard_value_balance > 0` **and** `ironwood_value_balance < 0`, with the sign convention verified against zebra's own documentation (positive means value removed from that pool, i.e. leaving). The Orchard side is a **net** test, so a V6 transaction whose Orchard spends are exactly offset by Orchard outputs (net zero, or net negative) while value enters Ironwood classifies as pass-through even though Orchard notes were spent in it. Confirmed on real wire bytes: `orchard_vb = 0` with `ironwood_vb = -240000` is a pass-through today. That is a false negative, the leaking direction. The question is not merely "widen to `>= 0`", which still leaves the net-negative case, but whether the migration arm should be **gross**: *the transaction has an Orchard bundle with at least one spend AND `ironwood_value_balance < 0`*. The current boundary is implemented as locked and pinned by one test, so a decision is a one-line change plus a test edit. **This needs a call from Taylor and Zooko.**
