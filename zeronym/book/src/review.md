...
**The upstream gate (retired).** This work initially sat behind one gate: the threat-model doc (Taylor and Zooko) was the upstream dependency, and the build was held until it landed and was signed off as safe to run. The shim proof of concept sat deliberately inside that gate: it classified and logged, diverting nothing. That gate has since been retired and the build proceeded; the shim and hub are shipped and run as attested enclaves ([roadmap](./roadmap.md) has the status). The questions below remain the review agenda, now read against running code rather than a design.
...
**RESOLVED, 2026-08-03/05.** Caution shipped in-enclave termination (`e2e_encryption { mode = "tls" }`: a Caddy runs inside the enclave, obtains the Let's Encrypt certificate itself, and terminates the wallet's TLS there, so the private key is enclave-born and the operator never holds it), and two days later the `upstream_protocol = "h2c"` field gRPC needs. The full path is verified end to end on deployed enclaves: wallet TLS into the enclave, h2c to the shim, TLS to the backend, with `GetLightdInfo` answering cleanly and trailers intact. The 2026-08-02 answer above stands as the record of why the platform default was the bad answer. The shim's own rustls/ACME stack remains the vendor-independent path and stays dormant on Caution.
...
- **Hub re-validation:** confirm the hub should re-parse and re-classify each tx, not trust the shim, and reject anything that is not an Orchard exit before batching (see [components](./components.md)).
...
- **The classifier, and what the proof of concept settled.** `is_orchard_exit` is now a pure function over the vendored `zebra-chain` parser, and fail-safe for privacy is implemented rather than proposed: every body the shim cannot read cleanly routes to the migration arm, each case pinned by a test ([components](./components.md) has the set). **Left open:** is that set complete, and does treating every unreadable body as a migration stay right now that diversion is destructive rather than a log line?
- **The classifier predicate: RESOLVED by Zooko, no longer an open question.** His ruling: *any transaction with an Orchard value balance above zero is a privacy risk to the user, regardless of the destination pool.* So the predicate is one conjunct, `orchard_value_balance > 0`, with the sign convention verified against zebra's own documentation (positive means value removed from that pool, i.e. leaving). The earlier `V6` and `ironwood_value_balance < 0` conjuncts are both gone, and the code matches ([components](./components.md)). The reasoning is the closed pool: NU6.3 forbids new value entering Orchard, so `orchard_vb >= 0` always holds post-activation, everyone still holding Orchard notes has held them since before activation, and **spending Orchard at all** is the identifying event, revealing that an IP controls legacy Orchard funds against a finite, shrinking set. Where the value lands afterwards does not change that inference, so the destination pool is irrelevant and an Orchard deshield to transparent is now batched exactly like an Orchard-to-Ironwood migration. The correction history is kept visible because the rest of this list should be read against it: an earlier draft of this bullet claimed the strict `> 0` left a false-negative window at net-zero *or net-negative* Orchard and proposed a **gross** test (an Orchard bundle with at least one spend). **That analysis was wrong and is retracted**, because it ignored NU6.3's cross-address restriction: the net-negative case is consensus-invalid and cannot occur, and a gross test would sweep in the same-receiver change Orchard still produces (the pool is closed to new value, not to activity). Zooko's rule then **dissolves** the leftover net-versus-gross boundary rather than answering it: `> 0` is an exit and is batched, `== 0` moved no Orchard value and is correctly passed through, `< 0` cannot be mined. There is no "cheap insurance" judgement call left to make. The consequence to accept, not to decide: the batched class is wider than *migrations*, so an Orchard deshield is delayed by a flush window too (why that costs little is in [the architecture](./architecture.md) and [honest limits](./trust.md)).
- **One boundary the ruling's criterion does not cover, and it is Zooko's call, not ours.** The criterion is about **value**: `orchard_value_balance > 0`, value leaving the pool. The rationale behind it is about **spending**: post-NU6.3, spending Orchard at all is the identifying event. Those two coincide everywhere except one shape, which we should name rather than let a code comment close. A transaction can spend legacy Orchard notes and still net to exactly zero (fee paid from transparent or Sapling, change back to the same receiver); it moves no value out, so it classifies `PassThrough` and is broadcast in the clear through the operator's indexer, and yet it publishes those legacy notes' nullifiers on the wire. **The question for Zooko:** is the criterion meant as written (value out), or does it extend to any Orchard **spend**? The predicate implements it as written and the code has not been changed to pre-empt the answer. This is not the retracted net-versus-gross analysis returning: that claimed a false-negative window at net-**negative** Orchard, which is consensus-invalid, and it stays retracted. What survives is narrower and real. Widening is not free either, and that is the trade to weigh: the test that would catch this shape is "an Orchard bundle with at least one spend", which also sweeps in the ordinary same-receiver change Orchard still produces, since the pool is closed to new value but not to activity. Tracked as open question 4 in `zeronym/shim/README.md`.
- **Compressed `SendTransaction`: policy conflict.** A compressed body never reaches the parser and lands in the fail-safe arm; because compression is negotiated, the PoC normalizes the indexer's advertised `grpc-accept-encoding` back to `identity` (see [components](./components.md) for the lever this denies the operator). **Is normalizing right, or should the shim strip the header and refuse a compressed `SendTransaction` outright?** **And what does a fail-safe cost?** [Components](./components.md) says a false positive only delays a normal transaction because the hub still broadcasts it, but the hub is also specified to reject anything that is not an Orchard exit before batching. Confirm which.
