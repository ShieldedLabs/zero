...
This chapter walks the migration path, but the same attestation is what makes the **attested edge** protections real: that the operator's front-end is exactly the reviewed shim code, and that its promise not to log requester IPs is verifiable rather than merely trusted. See [the overview](./overview.md) for what that edge protects, and [for review](./for-review.md) for the claims still awaiting sign-off.
