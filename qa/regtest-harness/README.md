...
| `spend-poison` | an on-chain spend the wallet did not create is discovered and recorded (signer wallet shields a setup-funded coinbase, deshields a non-coinbase receipt back, fresh seed recovery must see the spend); also exercises non-coinbase listing | hot-wallet false-unspent accumulation: address-scoped spend-check watermark skipping unfetched spend heights |
