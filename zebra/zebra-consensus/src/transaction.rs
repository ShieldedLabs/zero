...
fn verify_transparent_inputs_and_outputs(
    ...
    let script_checks = (0..tx.inputs().len()).map(move |input_index| {
    ...
    let scripts = futures::future::try_join_all(script_checks);
    let mut checks = AsyncChecks::new();
    checks.push("script", async move { scripts.await.map(|_| ()) });
    Ok(checks)
}
