...
//!
//! Replacement is random: a full cache evicts the slot chosen by a keyed
//! siphash of an insert counter, uniform over the slots and independent of
//! the inserted key, so an adversary who cannot guess the seed cannot
...
pub(super) struct VerifiedScripts {
    ...
    /// adversary cannot predict which entry an insert evicts.
...
struct Inner {
    ...
    slots: Vec<WtxId>,
    /// Counts inserts; hashed to choose the victim slot, so the choice is
    /// uniform over the slots rather than fixed per key.
    inserts: u64,
}
...
impl VerifiedScripts {
    ...
    pub(super) fn contains(&self, key: &WtxId) -> bool {
        let hit = self.lock().keys.contains(key);
    ...
    pub(super) fn insert(&self, key: WtxId) {
        let (inserted, evicted, size) = {
            ...
            if !inner.keys.insert(key) {
                ...
                if inner.slots.len() < self.capacity {
                    ...
                    let victim_index = self.victim_index(inner.inserts);
        ...
        if inserted {
            ...
            // The cast is lossless: `size` is at most `SCRIPT_CACHE_CAPACITY`,
            // far below f64's exact-integer range.
            metrics::gauge!("zebra.consensus.transaction.script_cache.entries").set(size as f64);
        }
    }

    /// The slot the `insert_count`th insert replaces when the cache is full.
    ///
    /// Hashing the counter rather than the inserted key keeps the choice
    /// uniform over the slots: a per-key victim would make the cache
    /// direct-mapped, letting two hot colliding keys evict each other forever.
    fn victim_index(&self, insert_count: u64) -> usize {
        // Casts are lossless: `capacity` is a usize, and the modulus keeps
        // the result below it.
        (self.siphasher.hash(&insert_count.to_le_bytes()) % self.capacity as u64) as usize
    }
...
mod tests {
    ...
    fn replacement_is_deterministic_with_a_fixed_seed() {
        ...
        // The next new insert is the fifth, so it evicts victim_index(5).
        let new_key = wtx_id(5);
        let victim = cache.lock().slots[cache.victim_index(5)];
