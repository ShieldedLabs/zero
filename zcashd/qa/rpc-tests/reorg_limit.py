...
# Must match MAX_REORG_LENGTH in src/main.h (which in turn matches Zebra's
# MAX_BLOCK_REORG_HEIGHT rollback window).
REORG_LIMIT = 1000
...
class ReorgLimitTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        print("Mine %d blocks on Node 0" % REORG_LIMIT)
        self.nodes[0].generate(REORG_LIMIT)
        assert(self.nodes[0].getblockcount() == 200 + REORG_LIMIT)
        assert(self.nodes[2].getblockcount() == 200)

        print("Mine competing %d blocks on Node 2" % (REORG_LIMIT + 1))
        self.nodes[2].generate(REORG_LIMIT + 1)
        assert(self.nodes[0].getblockcount() == 200 + REORG_LIMIT)
        assert(self.nodes[2].getblockcount() == 201 + REORG_LIMIT)

        print("Connect nodes to force a reorg")
        connect_nodes_bi(self.nodes, 0, 2)
        self.is_network_split = False
        # Disconnecting and reconnecting REORG_LIMIT blocks takes a while.
        sync_blocks(self.nodes, timeout=600)

        print("Check Node 0 is still running and on the correct chain")
        assert(self.nodes[0].getblockcount() == 201 + REORG_LIMIT)
        ...
        print("Mine %d blocks on Node 0" % (REORG_LIMIT + 1))
        self.nodes[0].generate(REORG_LIMIT + 1)
        assert(self.nodes[0].getblockcount() == base + REORG_LIMIT + 1)
        assert(self.nodes[2].getblockcount() == base)

        print("Mine competing %d blocks on Node 2" % (REORG_LIMIT + 2))
        self.nodes[2].generate(REORG_LIMIT + 2)
        assert(self.nodes[0].getblockcount() == base + REORG_LIMIT + 1)
        assert(self.nodes[2].getblockcount() == base + REORG_LIMIT + 2)
        ...
        try:
            ...
            # Node 0 has to download and header-check the heavier chain before
            # it hits the reorg-length check, so allow it some time to die.
            assert(check_stopped(0, timeout=60))

            # Check that node 0 stopped for the expected reason.
            self.log_stderr.seek(0)
            stderr = self.log_stderr.read().decode('utf-8')
            expected_msg = "A block chain reorganization has been detected that would roll back %d blocks!" % (REORG_LIMIT + 1)
