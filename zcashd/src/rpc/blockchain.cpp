...
UniValue z_gettreestate(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "    }\n"
            "  },\n"
            "  \"ironwood\": {           (object, optional) present whenever NU6.3 has a configured activation\n"
            "                            height for this chain. For blocks prior to NU6.3 activation, finalRoot\n"
            "                            is the null hash (all zeroes) and finalState/skipHash are absent.\n"
            "    \"skipHash\": \"hash\",   (string) hash of most recent block with more information\n"
            "    \"commitments\": {\n"
            "      \"finalRoot\": \"hex\", (string)\n"
            "      \"finalState\": \"hex\" (string)\n"
